...
-module(credo_elixir_tokenizer).
-include("credo_elixir.hrl").
-include("credo_elixir_tokenizer.hrl").
...
tokenize(String, Line, Column, #credo_elixir_tokenizer{} = Scope) ->
  ...
  Scope =
    lists:foldl(fun
      ({check_terminators, false}, Acc) ->
        Acc#credo_elixir_tokenizer{cursor_completion=false, terminators=none};
      ({check_terminators, {cursor, Terminators}}, Acc) ->
        Acc#credo_elixir_tokenizer{cursor_completion=prune_and_cursor, terminators=Terminators};
      ({existing_atoms_only, ExistingAtomsOnly}, Acc) when is_boolean(ExistingAtomsOnly) ->
        Acc#credo_elixir_tokenizer{existing_atoms_only=ExistingAtomsOnly};
      ({static_atoms_encoder, StaticAtomsEncoder}, Acc) when is_function(StaticAtomsEncoder) ->
        Acc#credo_elixir_tokenizer{static_atoms_encoder=StaticAtomsEncoder};
      ({preserve_comments, PreserveComments}, Acc) when is_function(PreserveComments) ->
        Acc#credo_elixir_tokenizer{preserve_comments=PreserveComments};
      ({unescape, Unescape}, Acc) when is_boolean(Unescape) ->
        Acc#credo_elixir_tokenizer{unescape=Unescape};
      ({indentation, Indentation}, Acc) when Indentation >= 0 ->
        Acc#credo_elixir_tokenizer{column=Indentation+1};
      (_, Acc) ->
        Acc
    end, #credo_elixir_tokenizer{identifier_tokenizer=IdentifierTokenizer}, Opts),
...
tokenize([], Line, Column, #credo_elixir_tokenizer{cursor_completion=Cursor} = Scope, Tokens) when Cursor /= false ->
  #credo_elixir_tokenizer{ascii_identifiers_only=Ascii, terminators=Terminators, warnings=Warnings} = Scope,
...
tokenize([], EndLine, EndColumn, #credo_elixir_tokenizer{terminators=[{Start, {StartLine, StartColumn, _}, _} | _]} = Scope, Tokens) ->
...
tokenize([], Line, Column, #credo_elixir_tokenizer{} = Scope, Tokens) ->
  #credo_elixir_tokenizer{ascii_identifiers_only=Ascii, warnings=Warnings} = Scope,
  AllWarnings = maybe_unicode_lint_warnings(Ascii, Tokens, Warnings),
...
tokenize([$0, $x, H | T], Line, Column, Scope, Tokens) when ?is_hex(H) ->
  ...
  Token = {int, {Line, Column, Number, Line, Column + 2 + Length}, OriginalRepresentation},
...
tokenize([$?, $\\, H | T], Line, Column, Scope, Tokens) ->
  Char = credo_elixir_interpolation:unescape_map(H),
...
tokenize(".:" ++ Rest, Line, Column, Scope, Tokens) when ?is_space(hd(Rest)) ->
  tokenize(Rest, Line, Column + 2, Scope, [{kw_identifier, {Line, Column, nil, Line, Column + 2}, '.'} | Tokens]);

tokenize("<<>>:" ++ Rest, Line, Column, Scope, Tokens) when ?is_space(hd(Rest)) ->
  tokenize(Rest, Line, Column + 5, Scope, [{kw_identifier, {Line, Column, nil, Line, Column + 5}, '<<>>'} | Tokens]);
tokenize("%{}:" ++ Rest, Line, Column, Scope, Tokens) when ?is_space(hd(Rest)) ->
  tokenize(Rest, Line, Column + 4, Scope, [{kw_identifier, {Line, Column, nil, Line, Column + 4}, '%{}'} | Tokens]);
tokenize("%:" ++ Rest, Line, Column, Scope, Tokens) when ?is_space(hd(Rest)) ->
  tokenize(Rest, Line, Column + 2, Scope, [{kw_identifier, {Line, Column, nil, Line, Column + 2}, '%'} | Tokens]);
tokenize("&:" ++ Rest, Line, Column, Scope, Tokens) when ?is_space(hd(Rest)) ->
  tokenize(Rest, Line, Column + 2, Scope, [{kw_identifier, {Line, Column, nil, Line, Column + 2}, '&'} | Tokens]);
tokenize("{}:" ++ Rest, Line, Column, Scope, Tokens) when ?is_space(hd(Rest)) ->
  tokenize(Rest, Line, Column + 3, Scope, [{kw_identifier, {Line, Column, nil, Line, Column + 3}, '{}'} | Tokens]);
tokenize("..//:" ++ Rest, Line, Column, Scope, Tokens) when ?is_space(hd(Rest)) ->
  tokenize(Rest, Line, Column + 5, Scope, [{kw_identifier, {Line, Column, nil, Line, Column + 5}, '..//'} | Tokens]);

tokenize(":<<>>" ++ Rest, Line, Column, Scope, Tokens) ->
  tokenize(Rest, Line, Column + 5, Scope, [{atom, {Line, Column, nil, Line, Column + 5}, '<<>>'} | Tokens]);
tokenize(":%{}" ++ Rest, Line, Column, Scope, Tokens) ->
  tokenize(Rest, Line, Column + 4, Scope, [{atom, {Line, Column, nil, Line, Column + 4}, '%{}'} | Tokens]);
tokenize(":%" ++ Rest, Line, Column, Scope, Tokens) ->
  tokenize(Rest, Line, Column + 2, Scope, [{atom, {Line, Column, nil, Line, Column + 2}, '%'} | Tokens]);
tokenize(":{}" ++ Rest, Line, Column, Scope, Tokens) ->
  tokenize(Rest, Line, Column + 3, Scope, [{atom, {Line, Column, nil, Line, Column + 3}, '{}'} | Tokens]);
tokenize(":..//" ++ Rest, Line, Column, Scope, Tokens) ->
  tokenize(Rest, Line, Column + 5, Scope, [{atom, {Line, Column, nil, Line, Column + 5}, '..//'} | Tokens]);
...
tokenize([$:, T1, T2, T3 | Rest], Line, Column, Scope, Tokens) when
  ...
  Token = {atom, {Line, Column, nil, Line, Column + 4}, list_to_atom([T1, T2, T3])},
...
tokenize([$:, $:, $: | Rest], Line, Column, Scope, Tokens) ->
  ...
  Token = {atom, {Line, Column, nil, Line, Column + 3}, '::'},
...
tokenize([$:, T1, T2 | Rest], Line, Column, Scope, Tokens) when
  ...
  Token = {atom, {Line, Column, nil, Line, Column + 3}, list_to_atom([T1, T2])},
...
tokenize([$:, T | Rest], Line, Column, Scope, Tokens) when
  ...
  Token = {atom, {Line, Column, nil, Line, Column + 2}, list_to_atom([T])},
  tokenize(Rest, Line, Column + 2, Scope, [Token | Tokens]);

% ## Stand-alone tokens

tokenize("=>" ++ Rest, Line, Column, Scope, Tokens) ->
  Token = {assoc_op, {Line, Column, previous_was_eol(Tokens), Line, Column + 2}, '=>'},
...
tokenize("..//" ++ Rest = String, Line, Column, Scope, Tokens) ->
  case strip_horizontal_space(Rest, 0) of
    {[$/ | _] = Remaining, Extra} ->
      Token = {identifier, {Line, Column, nil, Line, Column + 4 + Extra}, '..//'},
...
tokenize([$, | Rest], Line, Column, Scope, Tokens) ->
  Token = {',', {Line, Column, 0, Line, Column + 1}},
  tokenize(Rest, Line, Column + 1, Scope, [Token | Tokens]);

tokenize([$<, $< | Rest], Line, Column, Scope, Tokens) ->
  Token = {'<<', {Line, Column, nil, Line, Column + 2}},
  handle_terminator(Rest, Line, Column + 2, Scope, Token, Tokens);

tokenize([$>, $> | Rest], Line, Column, Scope, Tokens) ->
  Token = {'>>', {Line, Column, previous_was_eol(Tokens), Line, Column + 2}},
  handle_terminator(Rest, Line, Column + 2, Scope, Token, Tokens);
...
tokenize([T | Rest], Line, Column, Scope, Tokens) when T =:= $(; T =:= ${; T =:= $[ ->
  Token = {list_to_atom([T]), {Line, Column, nil, Line, Column + 1}},
  handle_terminator(Rest, Line, Column + 1, Scope, Token, Tokens);

tokenize([T | Rest], Line, Column, Scope, Tokens) when T =:= $); T =:= $}; T =:= $] ->
  Token = {list_to_atom([T]), {Line, Column, previous_was_eol(Tokens), Line, Column + 1}},
  handle_terminator(Rest, Line, Column + 1, Scope, Token, Tokens);

% ## Two Token Operators
tokenize([T1, T2 | Rest], Line, Column, Scope, Tokens) when ?ternary_op(T1, T2) ->
  Op = list_to_atom([T1, T2]),
  Token = {ternary_op, {Line, Column, previous_was_eol(Tokens), Line, Column + 2}, Op},
  tokenize(Rest, Line, Column + 2, Scope, add_token_with_eol(Token, Tokens));
...
tokenize([$& | Rest], Line, Column, Scope, Tokens) ->
  ...
  Token = {Kind, {Line, Column, nil, Line, Column + 1}, '&'},
  tokenize(Rest, Line, Column + 1, Scope, [Token | Tokens]);
...
tokenize([$:, H | T] = Original, Line, Column, BaseScope, Tokens) when ?is_quote(H) ->
  ...
  case credo_elixir_interpolation:extract(Line, Column + 2, Scope, true, T, H) of
      ...
      case unescape_tokens(Parts, Line, Column, NewScope) of
        {ok, [Part]} when is_binary(Part) ->
          case unsafe_to_atom(Part, Line, Column, Scope) of
            {ok, Atom} ->
              Token = {atom_quoted, {Line, Column, H, NewLine, NewColumn}, Atom},
        ...
        {ok, Unescaped} ->
          Key = case Scope#credo_elixir_tokenizer.existing_atoms_only of
            true  -> atom_safe;
            false -> atom_unsafe
          end,
          Token = {Key, {Line, Column, H, NewLine, NewColumn}, Unescaped},
          tokenize(Rest, NewLine, NewColumn, NewScope, [Token | Tokens]);
...
tokenize([$: | String] = Original, Line, Column, Scope, Tokens) ->
  case tokenize_identifier(String, Line, Column, Scope, false) of
    {_Kind, Unencoded, Atom, Rest, Length, Ascii, _Special} ->
      ...
      Token = {atom, {Line, Column, Unencoded, Line, Column + 1 + Length}, Atom},
      tokenize(Rest, Line, Column + 1 + Length, TrackedScope, [Token | Tokens]);
    empty when Scope#credo_elixir_tokenizer.cursor_completion == false ->
  ...
  case tokenize_number(T, [H], 1, false) of
    ...
    {[I | Rest], Number, Original, _Length} when ?is_upcase(I); ?is_downcase(I); I == $_ ->
      if
        ...
        Scope#credo_elixir_tokenizer.cursor_completion /= false ->
          tokenize([], Line, Column, Scope, Tokens);
    ...
    {Rest, Number, Original, Length} when is_integer(Number) ->
      Token = {int, {Line, Column, Number, Line, Column + Length}, Original},
      tokenize(Rest, Line, Column + Length, Scope, [Token | Tokens]);
    {Rest, Number, Original, Length} ->
      Token = {flt, {Line, Column, Number, Line, Column + Length}, Original},
      tokenize(Rest, Line, Column + Length, Scope, [Token | Tokens])
  end;
...
tokenize(";" ++ Rest, Line, Column, Scope, []) ->
  tokenize(Rest, Line, Column + 1, Scope, [{';', {Line, Column, 0, Line, Column + 1}}]);

tokenize(";" ++ Rest, Line, Column, Scope, [Top | _] = Tokens) when element(1, Top) /= ';' ->
  tokenize(Rest, Line, Column + 1, Scope, [{';', {Line, Column, 0, Line, Column + 1}} | Tokens]);
...
tokenize([$% | T], Line, Column, Scope, Tokens) ->
  tokenize(T, Line, Column + 1, Scope, [{'%', {Line, Column, nil, Line, Column + 1}} | Tokens]);

tokenize([$. | T], Line, Column, Scope, Tokens) ->
  tokenize_dot(T, Line, Column + 1, {Line, Column, nil, Line, Column + 1}, Scope, Tokens);
...
tokenize(String, Line, Column, OriginalScope, Tokens) ->
  case tokenize_identifier(String, Line, Column, OriginalScope, not previous_was_dot(Tokens)) of
    {Kind, Unencoded, Atom, Rest, Length, Ascii, Special} ->
      ...
      case Rest of
        [$: | T] when ?is_space(hd(T)) ->
          Token = {kw_identifier, {Line, Column, Unencoded, Line, Column + Length + 1}, Atom},
    ...
    empty when OriginalScope#credo_elixir_tokenizer.cursor_completion == false ->
...
tokenize_eol(Rest, Line, Scope, Tokens) ->
  {StrippedRest, Column} = strip_horizontal_space(Rest, Scope#credo_elixir_tokenizer.column),
  IndentedScope = Scope#credo_elixir_tokenizer{indentation=Column-1},
  ...
  case strip_horizontal_space(T, 0) of
    {[$# | R], _} ->
      case tokenize_comment(R, [$#]) of
        ...
        {Rest, Comment} ->
          preserve_comments(Line, Column, Tokens, Comment, Rest, Scope),
          tokenize_dot(Rest, Line, Scope#credo_elixir_tokenizer.column, DotInfo, Scope, Tokens)
      end;
    {"\r\n" ++ Rest, _} ->
      tokenize_dot(Rest, Line + 1, Scope#credo_elixir_tokenizer.column, DotInfo, Scope, Tokens);
    {"\n" ++ Rest, _} ->
      tokenize_dot(Rest, Line + 1, Scope#credo_elixir_tokenizer.column, DotInfo, Scope, Tokens);
    {Rest, Length} ->
...
handle_strings(T, Line, Column, H, Scope, Tokens) ->
  case credo_elixir_interpolation:extract(Line, Column, Scope, true, T, H) of
    {error, Reason} ->
      erlang:display({handle_strings1, Reason}),
      ...
      case unescape_tokens(Parts, Line, Column, NewScope) of
        {ok, [Part]} when is_binary(Part) ->
          case unsafe_to_atom(Part, Line, Column - 1, Scope) of
            {ok, Atom} ->
              Token = {kw_identifier, {Line, Column - 1, H, NewLine, NewColumn + 1}, Atom},
    ...
    {NewLine, NewColumn, Parts, Rest, InterScope} ->
      ...
      case unescape_tokens(Parts, Line, Column, NewScope) of
        {ok, Unescaped} ->
          ...
          Token = {string_type(H), {Line, Column - 1, nil, NewLine, NewColumn}, Unescaped},
          tokenize(Rest, NewLine, NewColumn, NewScope, [Token | Tokens]);
...
handle_unary_op([$: | Rest], Line, Column, _Kind, Length, Op, Scope, Tokens) when ?is_space(hd(Rest)) ->
  Token = {kw_identifier, {Line, Column, nil, Column + Length + 1}, Op},
...
handle_op([$: | Rest], Line, Column, _Kind, Length, Op, Scope, Tokens) when ?is_space(hd(Rest)) ->
  Token = {kw_identifier, {Line, Column, nil, Line, Column + Length + 1}, Op},
  tokenize(Rest, Line, Column + Length + 1, Scope, [Token | Tokens]);
...
handle_dot([$., H | T] = Original, Line, Column, DotInfo, BaseScope, Tokens) when ?is_quote(H) ->
  ...
  case credo_elixir_interpolation:extract(Line, Column + 1, Scope, true, T, H) of
...
handle_space_sensitive_tokens([Sign, NotMarker | T], Line, Column, Scope, [{identifier, _, _} = H | Tokens]) when
  ...
  Rest = [NotMarker | T],
  DualOpToken = {dual_op, {Line, Column, nil, Line, Column + length([Sign])}, list_to_atom([Sign])},
...
handle_space_sensitive_tokens([], Line, Column,
                              #credo_elixir_tokenizer{cursor_completion=Cursor} = Scope,
...
eol(_Line, _Column, [{eol, {Line, Column, Count}} | Tokens]) ->
  [{eol, {Line, Column, Count + 1, Line + 1, 1}} | Tokens];
eol(Line, Column, Tokens) ->
  [{eol, {Line, Column, 1, Line + 1, 1}} | Tokens].

is_unnecessary_quote([Part], Scope) when is_list(Part) ->
  case (Scope#credo_elixir_tokenizer.identifier_tokenizer):tokenize(Part) of
...
unescape_tokens(Tokens, Line, Column, #credo_elixir_tokenizer{unescape=true}) ->
  case credo_elixir_interpolation:unescape_tokens(Tokens) of
    {ok, Result} ->
      {ok, Result};
...
tokenize_identifier(String, Line, Column, Scope, MaybeKeyword) ->
  case (Scope#credo_elixir_tokenizer.identifier_tokenizer):tokenize(String) of
...
suggest_simpler_unexpected_token_in_error(Wrong, Line, WrongColumn, Scope) ->
  ...
  case (Scope#credo_elixir_tokenizer.identifier_tokenizer):tokenize(NFKC) of
    {error, _Reason} ->
       ConfusableSkeleton = 'Elixir.String.Tokenizer.Security':confusable_skeleton(Wrong),
       case (Scope#credo_elixir_tokenizer.identifier_tokenizer):tokenize(ConfusableSkeleton) of
         {_, Simpler, _, _, _, _} ->
...
tokenize_alias(Rest, Line, Column, Unencoded, Atom, Length, Ascii, Special, Scope, Tokens) ->
  if
    ...
    true ->
      AliasesToken = {alias, {Line, Column, Unencoded, Line, Column + Length}, Atom},
...
check_call_identifier(Line, Column, Info, Atom, [$( | _]) ->
  {paren_identifier, {Line, Column, Info, Line, Column + length(atom_to_list(Atom))}, Atom};
check_call_identifier(Line, Column, Info, Atom, [$[ | _]) ->
  {bracket_identifier, {Line, Column, Info, Line, Column + length(atom_to_list(Atom))}, Atom};
check_call_identifier(Line, Column, Info, Atom, _Rest) ->
  {identifier, {Line, Column, Info, Line, Column + length(atom_to_list(Atom))}, Atom}.
...
check_terminator({'end', {EndLine, _, _}}, [{'do', _, Indentation} | Terminators], Scope) ->
  NewScope =
    ...
    case Scope#credo_elixir_tokenizer.indentation > Indentation of
      true ->
        Hint = {'end', EndLine, Scope#credo_elixir_tokenizer.indentation},
        Scope#credo_elixir_tokenizer{mismatch_hints=[Hint | Scope#credo_elixir_tokenizer.mismatch_hints]};
...
missing_terminator_hint(Start, End, #credo_elixir_tokenizer{mismatch_hints=Hints}) ->
...
tokenize_keyword(token, Rest, Line, Column, Atom, Length, Scope, Tokens) ->
  Token = {Atom, {Line, Column, nil, Line, Column + Length}},
  tokenize(Rest, Line, Column + Length, Scope, [Token | Tokens]);

tokenize_keyword(block, Rest, Line, Column, Atom, Length, Scope, Tokens) ->
  Token = {block_identifier, {Line, Column, nil, Line, Column + Length}, Atom},
  tokenize(Rest, Line, Column + Length, Scope, [Token | Tokens]);
...
tokenize_keyword(Kind, Rest, Line, Column, Atom, Length, Scope, Tokens) ->
  NewTokens =
    case strip_horizontal_space(Rest, 0) of
      {[$/ | _], _} ->
        [{identifier, {Line, Column, nil, Line, Column + Length}, Atom} | Tokens];
      ...
      _ ->
        case {Kind, Tokens} of
          ...
          {_, _} ->
            add_token_with_eol({Kind, {Line, Column, previous_was_eol(Tokens), Line, Column + Length}, Atom}, Tokens)
        end
    end,
...
tokenize_sigil_contents([H | T] = Original, [S | _] = SigilName, Line, Column, Scope, Tokens)
    when ?is_sigil(H) ->
  case credo_elixir_interpolation:extract(Line, Column + 1, Scope, ?is_downcase(S), T, sigil_terminator(H)) of
...
add_sigil_token(SigilName, Line, Column, NewLine, NewColumn, Parts, Rest, Scope, Tokens, Indentation, Delimiter) ->
  ...
  case MaybeEncoded of
    {ok, Atom} ->
      ...
      Token = {sigil, {Line, TokenColumn, nil, NewLine, NewColumn + length(Modifiers)}, Atom, Parts, Modifiers, Indentation, Delimiter},
...
tokenize_keyword_terminator(Line, Column, do, Tokens) ->
  case is_valid_do(Tokens) of
    true  -> {ok, add_token_with_eol({do, {Line, Column, nil, Line, Column + 2}}, Tokens)};
    false -> {error, invalid_do_error("unexpected reserved word: "), "do"}
  end;
tokenize_keyword_terminator(Line, Column, Atom, Tokens) ->
  {ok, [{Atom, {Line, Column, nil, Line, Column + length(atom_to_list(Atom))}} | Tokens]}.
...
prepend_warning(Line, Column, Msg, #credo_elixir_tokenizer{warnings=Warnings} = Scope) ->
  Scope#credo_elixir_tokenizer{warnings = [{{Line, Column}, Msg} | Warnings]}.

track_ascii(true, Scope) -> Scope;
track_ascii(false, Scope) -> Scope#credo_elixir_tokenizer{ascii_identifiers_only=false}.
...
error(Reason, Rest, #credo_elixir_tokenizer{warnings=Warnings}, Tokens) ->
