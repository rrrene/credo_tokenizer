...
-module(credo_elixir_interpolation).
...
extract([], _Buffer, _Output, Line, Column, #credo_elixir_tokenizer{cursor_completion=false}, _Interpol, Last) ->
...
extract_nl(Rest, Buffer, Output, Line, Scope, Interpol, Last) ->
  extract(Rest, Buffer, Output, Line + 1, Scope#credo_elixir_tokenizer.column, Scope, Interpol, Last).
...
cursor_complete(Line, Column, Terminators) ->
  lists:mapfoldl(
    fun({Start, _, _}, AccColumn) ->
      End = credo_elixir_tokenizer:terminator(Start),
